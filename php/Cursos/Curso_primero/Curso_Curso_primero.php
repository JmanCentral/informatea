...
function listarArchivos($periodo) {
    ...
    while ($archivo = $result->fetch_assoc()) {
        ...
        echo '<a href="TareasSubidas.php?periodo=' . $archivo['periodo'] . '&tarea_id=' . $archivo['id'] . '" class="btn btn-warning btn-sm me-2">Ver Tareas Subidas</a>';
        echo '<a href="EliminarTarea.php?id=' . $archivo['id'] . '&archivo=' . urlencode($archivo['archivo']) . '" class="btn btn-danger btn-sm" onclick="return confirm(\'¿Estás seguro de eliminar esta tarea?\');">Eliminar</a>';
        echo '</div>';
        echo '</div>';
    }
    echo '</div>';
